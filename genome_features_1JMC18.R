...
# === 1. Cargar datos ===
charact <- read.table("feature_table_model_characteristics_1JMC18.tsv", header = TRUE)
format_table <- read.table("feature_table_characteristics_1JMC18.tsv", header = TRUE)

df<-cbind(format_table[,1:3],charact[,4:17])

breaks <- read.table("breakpoints_1JMC18.txt", header = TRUE, sep = "\t")
head(df)
...
df_rearr<- subset(df,Chromosome %in% c(1,4,6,8,10,24,32,35))  %>% arrange(Chromosome, Start)
...
# === 6. Combinar ===
combined_plot <- p2 / p4 / p6  /p12 / p7 / p5

# === 7. Mostrar ===
print(combined_plot)

ggsave(
  filename = "1JMC18_genome_features_100kb_genes_te_ltr_unk_gc.pdf",
  plot = combined_plot,
...
plot_feature_rearr <- function(df, yvar, color, fill, ylab, breaks) {
  ggplot(df, aes(x = GenomePos, y = !!sym(yvar), group = Chromosome)) +
    ...
    scale_color_manual(values=c("1"="lightblue", "4"="#CBB69C","6"="#FFD59A",
                         "8"="#D8BFD8","10"="#FFA07A","24"="#99FF66",
                         "32"="#FFD59A","35"="#CBB69C"))+
    # puntos
    geom_point(alpha = 0.2, size = 0.5) +
    # suavizado LOESS por cromosoma
...
ggsave(
  filename = "1JMC18_rearr_chr_genome_features_100kb_genes_te_ltr_unk_gc.pdf",
  plot = combined_plot,
  path = "C:/Users/inesg/Desktop/tesis/helodes/circus_graph",
  device = "pdf",
  width = 14,   
